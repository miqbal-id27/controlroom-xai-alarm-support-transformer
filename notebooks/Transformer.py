...
with open('Dataset/HDFS/hdfs_train', 'r') as f:
    for line in f:
        nums = list(map(int, line.strip().split()))
        if nums:
            max_event_id = max(max_event_id, max(nums))

PAD = 0
BOS = max_event_id + 1  # Avoid Semantic Collision
VOCAB_SIZE = BOS + 1
    ...
    with open('Dataset/' + filename, 'r') as f:        
        ...
        for j in range(nbatches):
            for i in range(batch):           
                ...
                t1[i, 1:] = torch.tensor(x, dtype=torch.long)
                t2[i, 1:] = torch.tensor(y, dtype=torch.long)
            ...
            src = Variable(t1.to(device), requires_grad=False)
            tgt = Variable(t2.to(device), requires_grad=False)
...
def test(model):
    ...
    for i in range(num_n):
        ...
        pred = greedy_decode(model, src, src_mask, tgt, max_len=WINDOW_SIZE+1, start_symbol=BOS, pred=False, g=9)
